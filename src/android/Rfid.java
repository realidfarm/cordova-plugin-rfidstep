...
public class Rfid extends CordovaPlugin implements DFRfid {
    ...
    public static long byteArrayToLong(byte[] arrByte, int index,boolean bLowerFirst) {
        long ret=0L;
        if (bLowerFirst) {
          for(int i=7;i>=0;i--){
            ret= (ret<<8) | (0xFFL & arrByte[i + index]);
          }
        } else {
          for(int i=0;i<8;i++){
            ret= (ret<<8) | (0xFFL & arrByte[i + index]);
    ...
    public static long toUInt32(byte[] arrByte, int index,boolean bLowerFirst) {
        long ret=0L;
        if (bLowerFirst) {
          for(int i=3;i>=0;i--){
            ret= (ret<<8) | (0xFFL & arrByte[i + index]);
